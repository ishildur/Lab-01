...
author: "Ishildur Ulva Aadnedottir"
date: "`r Sys.Date()`"
...
```{r Q3}
visitors %>% arrange(desc(n_2022)) %>% head(n=1)
```
...
**If there is one NA in the variable, then the answer to a mean or median calculation will be NA.**
...
Inside setting the has most visitors.
...
There are some extremes (a lot of visitors, holidays?) in the larger end which makes the mean larger.
